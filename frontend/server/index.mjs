...
app.post('/api/voice', async (req, res) => {
  ...
  try {
    ...
    let userContent = `The question to ask the patient: "${question}"`
    if (Array.isArray(options) && options.length > 0) {
      // We deliberately do NOT send the option text — the patient picks from
      // on-screen buttons, so the reply must ask the question WITHOUT listing them.
      userContent +=
        `\n(The patient answers by tapping on-screen buttons — do NOT list, name, ` +
        `or hint at the options; ask the question only.)`
    }
    ...
    if (progressHint) {
      userContent += `\n\n(You may add a brief "almost there" — but only rarely, never every turn.)`
    }
